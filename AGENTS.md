...
All constants (config keys, defaults, platform list, HA weather-state enum) live in `const.py` — check there first before adding a new config field.
...
Covers `coordinator.py`'s gating/cooldown/run-limit logic, `config_flow.py`'s unique_id/duplicate-detection, reconfigure, and the options-merge fix, and `__init__.py`'s version migration. Entity platform files (`switch.py`, `number.py`, `sensor.py`, `binary_sensor.py`) aren't covered — they're thin `CoordinatorEntity` views with little logic of their own; extend `conftest.py`'s stubs (entity base classes, `entity_platform`, `restore_state`) if that changes.
