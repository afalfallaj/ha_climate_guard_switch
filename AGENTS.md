...
`ConfigFlow.async_step_user` is a menu: `guard` (the original form, moved verbatim to `async_step_guard`) or `history`. A history entry carries `entry_type: "history"` in its data (`is_history_entry()` in `history.py`); **guard entries have no `entry_type` key and absence means guard**, so no migration or version bump was needed and the guard path is untouched. History entries have **no coordinator, no `runtime_data`, no target switch and no `unique_id`** (their inputs are editable in options, so an id derived from them would go stale; several views are legitimate). `__init__.py`, `sensor.py`, `diagnostics.py` and `config_flow.py` each branch on `is_history_entry()` first — anything that assumes `entry.runtime_data` is a coordinator or reads `CONF_TARGET_ENTITY` must not be reached for a history entry. `HISTORY_PLATFORMS` is sensor only.

Purpose: let HA's *built-in* cards show a temperature, its target and the heating/cooling periods in ONE chart for any date range. HA draws values of one kind per chart and keeps long-term statistics only for sensors, so `sensor.py` turns the inputs into temperature sensors (`state_class = measurement`, `device_class = temperature`, system unit): `HistoryTargetTemperatureSensor` (the thermostat's target) and `HistoryTraceSensor` (keys `temperature_while_heating` / `temperature_while_cooling`: the temperature only while the relay is on, `None` = state `unknown` otherwise). The pure logic lives in `history.py`. Rules to keep:
- **Read-only, never a service call.** Inputs may come from any integration; the physical relays are the recommended heating/cooling inputs because they are what actually ran.
- Traces are ordinary visible sensors (owner's choice; note that a History-page device pick joins the statistics segments across idle hours, which the README states). Cards give them a per-entity `color`. README recipes must pin `period: hour` — day/month rows are adjacent and join.
- No fake values: a trace is `None` when idle and unavailable when either source is unavailable.
...
- v0.0.4 shipped a read-only climate entity (built-in shaded chart) and Heating/Cooling % sensors, both removed as not needed for the graph; v0.0.5 registered the traces hidden + diagnostic. The registry keeps `hidden_by`/`entity_category` from the first registration, so `__init__.py`'s `_async_tidy_history_registry` deletes the retired entries (unique ids `<entry_id>_history/_heating/_cooling`) and clears hidden/diagnostic on the traces when the integration set them (never a user's `hidden_by`), every time a history entry is set up — keep that until no v0.0.4/v0.0.5 installs remain.
...
Covers `coordinator.py`'s gating/cooldown/run-limit logic, `config_flow.py`'s unique_id/duplicate-detection, entry-type menu, reconfigure, and the options-merge fix (guard and history), `__init__.py`'s version migration and its guard-vs-history setup/unload split, `diagnostics.py`, translation coverage (`tests/test_translations.py`), and the whole History view (`history.py`'s helpers and the history sensors in `sensor.py`) in `tests/test_history.py`. `conftest.py` provides real minimal stand-ins where behavior matters (entity base classes, a `TemperatureConverter` that raises like the real one, a tiny entity registry) — keep them faithful when extending. The guard's thin `CoordinatorEntity` views (`switch.py`, `number.py`, `binary_sensor.py`, and `GuardStatusSensor` beyond its setup wiring) aren't covered, as they have little logic of their own; extend `conftest.py`'s stubs (`restore_state`, number/switch/binary_sensor bases) if that changes.
