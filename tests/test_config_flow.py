...
Covers the unique_id/duplicate-detection and reconfigure additions, locks in
the options-flow merge fix (options-flow save used to silently wipe the number
entities' run_limit/cooldown values — see AGENTS.md), the optional temperature
sensor for the history chart sensors, and the rejection of leftover "History
view" entries from v0.0.4–v0.0.6.
"""
...
def _leftover_history_entry() -> _ConfigEntry:
    return _ConfigEntry(data={CONF_ENTRY_TYPE: ENTRY_TYPE_HISTORY, CONF_TEMPERATURE_SENSOR: "sensor.t"})
...
async def test_user_step_stores_the_optional_temperature_sensor() -> None:
    flow = ConfigFlow()

    result = await flow.async_step_user(_user_input(**{CONF_TEMPERATURE_SENSOR: "sensor.room_temperature"}))

    assert result["data"][CONF_TEMPERATURE_SENSOR] == "sensor.room_temperature"
...
async def test_options_flow_clears_optional_fields_left_empty() -> None:
    """An emptied optional field is omitted by the frontend; None must override the saved value."""
    entry = _ConfigEntry(
        data=_user_input(**{CONF_SUN_ENTITY: "sun.sun", CONF_CLIMATE_ENTITY: "climate.t", CONF_TEMPERATURE_SENSOR: "sensor.t"})
    )
    flow = OptionsFlowHandler()
    flow.config_entry = entry

    result = await flow.async_step_init({CONF_TARGET_ENTITY: TARGET_ENTITY})

    assert result["data"][CONF_SUN_ENTITY] is None
...
async def test_options_flow_refuses_a_leftover_history_entry() -> None:
    flow = OptionsFlowHandler()
    flow.config_entry = _leftover_history_entry()

    result = await flow.async_step_init(None)

    assert result["type"] == "abort"
    assert result["reason"] == "history_view_removed"
...
async def test_reconfigure_refuses_a_leftover_history_entry() -> None:
    flow = ConfigFlow()
    flow._reconfigure_entry = _leftover_history_entry()
