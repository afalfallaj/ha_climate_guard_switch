...
async def async_setup_entry(hass: HomeAssistant, entry: GuardSwitchConfigEntry) -> bool:
    ...
    if is_history_entry(entry):
        # A leftover "History view" entry from v0.0.4–v0.0.6. Its sensors now live
        # on the guard device, so the entry can only be deleted; the message says so.
        raise ConfigEntryError(translation_domain=DOMAIN, translation_key="history_view_removed")
